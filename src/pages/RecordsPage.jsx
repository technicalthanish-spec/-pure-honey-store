...
function Records({kind}){
 ...
 const visible=rows.filter(r=>purchase||Boolean(r.deleted_at)===showDeleted)
 const total=rupees(visible.reduce((n,r)=>n+cents(purchase?r.total_cost:r.amount),0))
 ...
 <section className="admin-card"><div className="card-title-row"><div><h2>All-time records · {currency(total)}</h2><p>{visible.length} entries</p></div><button className="secondary-btn" disabled={!visible.length} onClick={()=>exportCsv(purchase?'stock-purchases':'business-expenses',visible)}>Download CSV</button></div>{!purchase&&<label className="toggle-row"><input type="checkbox" checked={showDeleted} onChange={e=>setShowDeleted(e.target.checked)}/> Show deleted expenses (excluded from profit)</label>}{loading?<p>Loading…</p>:<div className="table-wrap"><table><thead><tr><th>Date</th><th>{purchase?'Supplier / Product':'Category'}</th>{purchase&&<><th>Jars</th><th>Cost / jar</th></>}<th>Amount</th><th>Note</th><th>Actions</th></tr></thead><tbody>{visible.map(r=><tr key={r.id}><td>{shortDate((purchase?r.purchased_on:r.spent_on)+'T12:00:00+05:30')}</td><td>{purchase?r.supplier+' · '+r.size_label:r.category}</td>{purchase&&<><td>{r.quantity}</td><td>{currency(r.unit_cost)}</td></>}<td>{currency(purchase?r.total_cost:r.amount)}</td><td>{r.note}</td><td><RecordEditor kind={purchase?'purchase':'expense'} record={r} onSaved={load}/></td></tr>)}</tbody></table>{!visible.length&&<p>No records yet.</p>}</div>}</section>
 </>}
 </div>
}
