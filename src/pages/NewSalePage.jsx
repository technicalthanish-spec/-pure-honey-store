...
function SaleForm(){
 ...
 return <div className="admin-page">
  <div className="page-header"><div><div className="eyebrow">ADMIN BILLING</div><h1>New sale</h1><p>Create the order, invoice and payment from one screen.</p></div><Link className="secondary-btn" to="/admin/customers">Customer khata</Link></div>
  ...
  <form onSubmit={submit}><fieldset disabled={request.busy||Boolean(request.pending)}>
   <div className="detail-grid"><section className="admin-card"><h2>Customer</h2>
    ...
    <label className="field"><span>Customer name</span><input required minLength="2" maxLength="100" value={customer.name} onChange={e=>setCustomer({...customer,name:e.target.value})}/></label>
    <label className="field"><span>Mobile · identifies this customer's khata</span><input required inputMode="numeric" pattern="[0-9]{10}" maxLength="10" value={customer.mobile} onChange={e=>setCustomer({...customer,mobile:e.target.value})}/></label>
   ...
   </section><section className="admin-card"><h2>Honey and price</h2>
   ...
   <div className="detail-grid"><section className="admin-card"><h2>Discount and payment</h2><div className="form-grid">
   ...
   </section><section className="admin-card"><h2>Bill summary</h2><div className="detail-list">{[['Amount',amounts.subtotal],['Discount',amounts.discount],['Delivery',Number(bill.delivery)],['Final total',amounts.total],['Received',Number(bill.paid)],['Pending / udhaar',amounts.pending]].map(([k,v])=><div key={k}><span>{k}</span><strong>{currency(v)}</strong></div>)}</div><p className="muted-text">Stock reduces once when saved. Record only payment you have received. Remaining money stays in the customer's khata.</p><button className="primary-btn" disabled={!items.length||amounts.pending<0||amounts.total<0}>{request.busy?'Saving…':'Save sale & create invoice'}</button></section></div>
  </fieldset></form>
 </div>
}
